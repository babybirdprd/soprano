...
fn main() -> Result<()> {
    ...
    let vocos_config_path = "vocos_config.json";
    let vocos_weights_path = "decoder.safetensors";
    ...
    for (s_idx, s_text) in sentences.iter().enumerate() {
        println!(
            "Processing sentence {}/{}: {}",
            s_idx + 1,
            sentences.len(),
        ...
        let prompt = format!("[STOP][TEXT]{}[START]", s_text);
        let tokens = tokenizer.encode(prompt, false).map_err(E::msg)?;
        let mut current_input = Tensor::new(tokens.get_ids(), &device)?.unsqueeze(0)?;
        ...
        if generated_hidden_states.is_empty() {
            println!("Warning: No tokens generated for sentence {}", s_idx + 1);
            continue;
        }
